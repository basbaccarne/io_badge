...
Let's start at the beginning. If we want to create an animation, we need to first design and animate a concept. I believe that Figma has the best balance between creative flexibility and intuitive controls. For more advanced animations you can work in Adobe After Effect, Krita, etc. 
...
best in handling GIF and PNG spritesheets (using ```pygame.image.load()```)
* The [Python Script](tests/pygame_gif.py) reads the GIF and plays the frames at a set framerate